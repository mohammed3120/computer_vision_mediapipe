...
while(True):
    ...
    if len(lms_list) !=0:
         ...
         vol = np.interp(length, [25,150],[-65.25,0])
    ...
    cv2.putText(img,f'{ int(volPer) } %',(40, 450),cv2.FONT_HERSHEY_DUPLEX,1,(0,255,0),3)
        
    cTime = time.time()
    fps = 1/(cTime-pTime)
    pTime = cTime
    cv2.putText(img,f'fps: {int(fps)}',(10, 70),cv2.FONT_HERSHEY_DUPLEX,2,(0,255,255),3)
